...
class BufferedSocket(object):
    ...
    def __init__(self, sock: socket.socket, buffer_size: int = 256):
        ...
        self._buffer      = b''
        sock.settimeout(BufferedSocket.SOCKET_TIMEOUT_SECS)
    
    def send(self, data: bytes, flags: int = 0) -> int:
        global logger
        ret = -1
        try:
            ret = self._socket.send(data, flags)
        except Exception as e:
            logger.warning('Error on send: {}'.format(e))
        return ret
    ...
    def recv(self, num_bytes: int = 256, flags: int = 0) -> bytes:
        ...
        if len(self._buffer) < num_bytes:
            try:
                ...
                logger.warning('Error on recv: {}'.format(e))
